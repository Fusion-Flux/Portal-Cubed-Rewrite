...
public record DisintegrationSoundType(HolderSet<EntityType<?>> entities, HolderSet<SoundEvent> sounds) {
	public static final Codec<DisintegrationSoundType> DIRECT_CODEC = RecordCodecBuilder.create(instance -> instance.group(
			RegistryCodecs.holderSet(Registries.ENTITY_TYPE).fieldOf("entities").forGetter(DisintegrationSoundType::entities),
			RegistryCodecs.holderSet(Registries.SOUND_EVENT).fieldOf("sounds").forGetter(DisintegrationSoundType::sounds)
