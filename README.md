# Portal Cubed Rewrite

![under_construction_small](https://github.com/user-attachments/assets/eaa96425-2cb3-4e55-9eca-7276111dd28c)

# UNDER CONSTRUCTION

Portal Cubed is a Work-in-Progress Portal Mod for Fabric 26.3 that aims to bring many of the core gameplay mechanics and building blocks from Valve's Portal series to Minecraft.  It is currently being remade from the ground up, and is in Alpha.

As of Alpha 2, the list of features includes:
- The Aperture Science Handheld Portal Device
- The Long Fall Boots and Advanced Knee Replacements
- Prop entities, such as cubes, radios, and various other junk found around Aperture Laboratories
- Floor Buttons and Pedestal Buttons, including the Old Aperture and Portal 1 versions
- A large variety of test chamber building materials from all visual styles/time periods seen in the Portal series
- Various other decoration items and building blocks from across Aperture
- The Construction Cannon, a new tool to assist in the building of test chambers
- The totally legitimate test completion reward, the Black Forest Cake

...And a lot more on the way!




